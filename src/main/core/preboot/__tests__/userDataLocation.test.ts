...
describe('getNormalizedExecutablePath', () => {
  it('macOS: returns app.getPath("exe") verbatim', async () => {
    ...
    stubElectron({ exePath: '/Applications/SuperAgent.app/Contents/MacOS/SuperAgent' })
    stubBootConfig()
    stubFs()
    const { getNormalizedExecutablePath } = await loadModule()
    expect(getNormalizedExecutablePath()).toBe('/Applications/SuperAgent.app/Contents/MacOS/SuperAgent')
  })
  ...
  it('Linux without APPIMAGE env: returns app.getPath("exe") verbatim', async () => {
    ...
    stubElectron({ exePath: '/usr/bin/superagent' })
    stubBootConfig()
    stubFs()
    const { getNormalizedExecutablePath } = await loadModule()
    expect(getNormalizedExecutablePath()).toBe('/usr/bin/superagent')
  })

  it('Linux with APPIMAGE env: returns normalized AppImage path', async () => {
    vi.stubEnv('APPIMAGE', '/home/alice/Applications/SuperAgent-1.0.0.AppImage')
    stubConstants({ isLinux: true, isWin: false, isPortable: false })
    stubElectron({ exePath: '/tmp/.mount_xxxx/usr/bin/superagent' })
    stubBootConfig()
    stubFs()
    const { getNormalizedExecutablePath } = await loadModule()
    // path.join is globally mocked to args.join('/'); path.dirname is real.
    expect(getNormalizedExecutablePath()).toBe('/home/alice/Applications/superagent.appimage')
  })

  it('Windows non-portable: returns app.getPath("exe") verbatim', async () => {
    stubConstants({ isLinux: false, isWin: true, isPortable: false })
    stubElectron({ exePath: 'C:\\Program Files\\SuperAgent\\SuperAgent.exe' })
    stubBootConfig()
    stubFs()
    const { getNormalizedExecutablePath } = await loadModule()
    expect(getNormalizedExecutablePath()).toBe('C:\\Program Files\\SuperAgent\\SuperAgent.exe')
  })

  it('Windows portable: returns PORTABLE_EXECUTABLE_DIR/superagent-portable.exe', async () => {
    vi.stubEnv('PORTABLE_EXECUTABLE_DIR', 'D:\\PortableApps\\SuperAgent')
    stubConstants({ isLinux: false, isWin: true, isPortable: true })
    stubElectron({ exePath: 'D:\\PortableApps\\SuperAgent\\SuperAgent.exe' })
    stubBootConfig()
    stubFs()
    const { getNormalizedExecutablePath } = await loadModule()
    // path.join is globally mocked to args.join('/').
    expect(getNormalizedExecutablePath()).toBe('D:\\PortableApps\\SuperAgent/superagent-portable.exe')
  })
})
...
describe('resolveUserDataLocation', () => {
  describe('normal resolution (no pending relocation)', () => {
    ...
    it('BootConfig empty + isPortable=true: setPath called with portableDir/data', async () => {
      ...
      const { resolveUserDataLocation } = await loadModule()
      resolveUserDataLocation()
      expect(setPathMock).toHaveBeenCalledWith('userData', 'D:\\PortableApps\\SuperAgent/data')
      expect(setPathMock).toHaveBeenCalledTimes(1)
    })
    ...
    it('AppImage normalized key matches in BootConfig: setPath called', async () => {
      vi.stubEnv('APPIMAGE', '/home/alice/Apps/SuperAgent-1.0.0.AppImage')
      stubConstants({ isLinux: true, isWin: false, isPortable: false })
      stubElectron({ exePath: '/tmp/.mount_abc/usr/bin/superagent' })
      // Key matches the *normalized* path, not raw exe.
      stubBootConfig({
        'app.user_data_path': {
          '/home/alice/Apps/superagent.appimage': '/home/alice/cherry-data'
        }
      })
      stubFs({ existsSyncImpl: () => true, accessSyncImpl: () => undefined })
      const { resolveUserDataLocation } = await loadModule()
    ...
    it('Windows portable normalized key matches in BootConfig: setPath called', async () => {
      ...
      stubBootConfig({
        'app.user_data_path': {
          'D:\\PortableApps\\SuperAgent/superagent-portable.exe': 'D:\\Data\\Cherry'
        }
      })
      stubFs({ existsSyncImpl: () => true, accessSyncImpl: () => undefined })
      const { resolveUserDataLocation } = await loadModule()
