---
title: Cloud login explains when SuperAgent needs an update
category: changed
severity: notice
...
When Cherry Cloud rejects an unsupported client version, the login prompt now asks the user to update SuperAgent instead of retrying the same login.
...
Update SuperAgent before signing in again.
