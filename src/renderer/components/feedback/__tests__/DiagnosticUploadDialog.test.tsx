...
const mocks = vi.hoisted(() => ({
  ...
  translations: {
    ...
    'settings.about.diagnostics.report.acknowledgement':
      'I understand that the problem description and selected diagnostic data may contain sensitive information, and agree to send this content to SuperAgent for troubleshooting.',
    ...
    'settings.about.diagnostics.upload.unknown.description':
      'Could not confirm whether this submission succeeded. Save the diagnostic file locally, then contact the SuperAgent support team or upload it through the Feishu form via Manual feedback. Retrying may submit the same diagnostic report again.',
...
const fallbackPath = AbsoluteFilePathSchema.parse('/tmp/superagent-diagnostics.zip')
...
const submissionFailedResult: Extract<OutputFor<'diagnostics.bundle.upload'>, { status: 'submission_failed' }> = {
  bundleId,
  fileName: 'superagent-diagnostics.zip',
...
async function completeReview(user: ReturnType<typeof userEvent.setup>, description = '  App freezes on launch.  ') {
  ...
  await user.click(
    screen.getByRole('checkbox', {
      name: 'I understand that the problem description and selected diagnostic data may contain sensitive information, and agree to send this content to SuperAgent for troubleshooting.'
    })
  )
