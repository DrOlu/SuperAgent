...
vi.mock('@shared/data/presets/miniApps', async (importOriginal) => ({
  ...
  BUILTIN_MINI_APPS: [
    { appId: 'com.superagent.miniapp.notes', name: { en: 'Notes' }, icon: 'icon.webp' },
    { appId: 'com.superagent.miniapp.draw', name: { en: 'Draw' }, icon: 'icon.webp' }
  ]
}))
...
vi.mock('@cherrystudio/ui', async () => {
  const actual = await vi.importActual<typeof SuperAgentUI>('@cherrystudio/ui')
...
describe('MiniAppsPage', () => {
  ...
  it('drops installed builtins from the offer', () => {
    ...
    const installed = ['com.superagent.miniapp.notes', 'com.superagent.miniapp.draw'].map((appId) =>
  ...
  it('does not re-offer an installed builtin the user disabled', () => {
    ...
    const hidden = ['com.superagent.miniapp.notes', 'com.superagent.miniapp.draw'].map((appId) =>
