 ...
 * userData directory via `~/.superagent/config/config.json`. On the
...
const DB_NAME = 'superagent.sqlite'
 ...
 *   2. Read `~/.superagent/config/config.json` for a legacy `appDataPath`.
...
export interface SelectionProbe {
  ...
  /** Directory already holds a non-empty superagent.sqlite (A0 guard). */
...
function isWindowsPortableExecutable(executablePath: string): boolean {
  ...
  return normalized.slice(normalized.lastIndexOf('/') + 1).toLowerCase() === 'superagent-portable.exe'
}
...
/**
 * Whether a directory already holds a *non-empty* superagent.sqlite. Mirrors
