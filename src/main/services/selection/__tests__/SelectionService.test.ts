...
const { getApplicationIdMock } = vi.hoisted(() => ({
  getApplicationIdMock: vi.fn(() => 'com.hyperspace.SuperAgent')
}))
...
describe('SelectionService macOS toolbar', () => {
  ...
  it.each([
    ...
    ['China', 'com.cherryai.superagent.cn']
  ...
  it('treats the other edition as an external app', () => {
    getApplicationIdMock.mockReturnValue('com.hyperspace.SuperAgent')
    const { access, toolbarWindow } = createToolbarHarness()

    access.showToolbarAtPosition({ x: 10, y: 20 }, 'bottomLeft', 'com.cherryai.superagent.cn')
