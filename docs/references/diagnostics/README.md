...
```bash
# macOS
CS_DIAGNOSTICS=1 "/Applications/SuperAgent.app/Contents/MacOS/SuperAgent"

# Windows (PowerShell) — adjust the path to your install location
$env:CS_DIAGNOSTICS=1; & "$env:LOCALAPPDATA\Programs\SuperAgent\SuperAgent.exe"

# Linux (AppImage; or the installed binary, whose path varies by distro)
CS_DIAGNOSTICS=1 "./SuperAgent-<version>-<arch>.AppImage"
```

### Output

Both dev and packaged runs write to the platform **app logs directory** exposed as `application.getPath('app.logs')` (dev diverts its logs like it does userData, using the same suffix — `Dev` by default, replaced by `CS_DEV_USER_DATA_SUFFIX` when set — so on macOS packaged runs use `~/Library/Logs/SuperAgent/` and dev runs use `~/Library/Logs/SuperAgentDev/`): signals stream into `app.<date>.log`, and the CPU profile lands beside it as `boot-whenReady.cpuprofile`.
