...
Automated tests are triggered on pull requests (PRs) opened by SuperAgent organization members, excluding draft PRs. PRs from new contributors are initially labeled `needs-ok-to-test` and are not automatically tested. After a SuperAgent organization member adds `/ok-to-test` to the PR, the test pipeline will be created.
...
v2 is the next major milestone for SuperAgent, and we invite every developer to actively participate! Whether it's new feature development, architecture optimization, or code refactoring, contributions on `main` are welcome. Let's build a better SuperAgent together!
