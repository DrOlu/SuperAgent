...
/**
 * Require this process to be the primary SuperAgent instance.
 *
 * Claims Electron's single-instance lock via `app.requestSingleInstanceLock()`.
 * If another SuperAgent process already holds the lock, this function
...
export function requireSingleInstance(): void {
  ...
  logger.info('Another SuperAgent instance already holds the single-instance lock; exiting')
  application.quit()
  process.exit(0)
}
