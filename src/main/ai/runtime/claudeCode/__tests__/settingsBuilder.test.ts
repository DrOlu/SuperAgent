...
describe('buildClaudeCodeSessionSettings', () => {
  ...
  it('gates Support and Assistant Bash per mode: ask interactively, lifted by bypassPermissions, denied headless', async () => {
    ...
    const directGhCommand = 'gh issue create --repo DrOlu/SuperAgent --title "Bug" --body-file report.md'
    const bashCommands = [
      directGhCommand,
      `bash -lc 'gh issue create --repo DrOlu/SuperAgent --title "Bug" --body-file report.md'`,
      'pnpm test'
    ]
  ...
  it('restricts Cherry Support to its bundled skills without marketplace access', async () => {
    ...
    mocks.loadBuiltinAgentDefinition.mockReturnValue({
      skills: ['cherry-assistant-guide', 'faq-collector', 'superagent-feedback', 'issue-reporter']
    })
    mocks.getBuiltinAgentPluginDirectory.mockReturnValue('/app/feature.agents.builtin/cherry-assistant/.claude')
    const session = {
      id: 'session-1',
    ...
    expect(settings.skills).toEqual([
      ...
      'cherry-assistant-builtin:superagent-feedback',
  ...
  it('does not inject a SuperAgent Assistant-only contract on every submitted prompt', async () => {
    mocks.getAgent.mockReturnValue({
      id: 'agent-1',
      type: 'claude-code',
      model: 'anthropic::claude-sonnet',
