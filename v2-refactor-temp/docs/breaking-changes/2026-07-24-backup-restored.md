...
The existing local, WebDAV, Nutstore, and S3 backup settings are enabled again. Newly created direct archives continue to use format version 7 and can be either full or slim: full archives contain `Data`, `IndexedDB`, `Local Storage`, and `cache.json`, while slim archives contain only `Data/superagent.sqlite` and `cache.json`. `metadata.json` records the selected resource layout, and no duplicate top-level database or `.claude` resource is created.
...
Create a fresh backup after upgrading, and choose a full backup when browser storage and all supported `Data` contents must be preserved. Use a slim backup when only the SQLite database and cache are required. SuperAgent v1 backup formats — version 6 direct ZIPs, metadata-less version 1-5 ZIPs, and `.bak` files — remain rejected.

## Notes for release manager

The `resources` flags distinguish full and slim layouts without increasing the archive version. Earlier version 7 archives with standalone SQLite and `.claude` resources remain restorable. The backup checkpoints SQLite before copying, so committed data is sealed in `superagent.sqlite`; SQLite recreates the excluded `superagent.sqlite-wal` and `superagent.sqlite-shm` sidecars after restore.
