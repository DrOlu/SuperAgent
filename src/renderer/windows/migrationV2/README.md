...
4. Exporters:
   - `ReduxExporter` scans the Redux Persist payload in `localStorage` (`persist:superagent`) and writes only migration-owned slices to separate files in bounded chunks.
...
outside SuperAgent support. Saving never uploads or attaches the bundle; metadata-only fallback is disclosed
when logs cannot be included. After a successful local-only save, the only support actions reveal the file and
copy `support@superagent.ng`; no mail client or prefilled email is provided. The V1 dialog also opens
