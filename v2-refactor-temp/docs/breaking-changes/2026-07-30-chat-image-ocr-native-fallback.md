...
When an image is attached in Home Chat and addressed to a model without declared image-recognition support, SuperAgent still runs OCR first — but if OCR finds no text, is not configured, or fails, the image is now forwarded to the provider as a base64-backed native image instead of being replaced with a `[could not read this file]` note.
