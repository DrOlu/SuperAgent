// =============================================================================
// LIVE SSH integration harness — drives the REAL CompanionManager + SshTransport
...
// electron `app` is a path string outside the electron runtime; companionArtifacts
...
const KEY = (process.env.CATE_LIVE_SSH_KEY ?? join(homedir(), '.ssh', 'id_ed25519'))
  .replace(/^~(?=$|\/)/, homedir())
const COMPANION_ID = process.env.CATE_LIVE_SSH_ID ?? 'srv_live'

/** Count live companion daemons on the server — one per live transport, so any
...
describe.skipIf(!LIVE)('live SSH companion (real server)', () => {
  let CompanionManagerCtor: typeof import('./companionManager').CompanionManager
  let SshTransportCtor: typeof import('./transports/sshTransport').SshTransport

  beforeAll(async () => {
    CompanionManagerCtor = (await import('./companionManager')).CompanionManager
    SshTransportCtor = (await import('./transports/sshTransport')).SshTransport
  })

  type Ev = { t: number; phase: string; message?: string }
  function harness(): { mgr: InstanceType<typeof CompanionManagerCtor>; events: Ev[]; t0: number } {
    const mgr = new CompanionManagerCtor()
    const events: Ev[] = []
    const t0 = Date.now()
  ...
  test('concurrent ensure x4 + a racing install opens exactly ONE server session', async () => {
    ...
    // reach for the companion at once, plus an install racing them.
    await Promise.all([
    ...
    // eslint-disable-next-line no-console
    console.log(`[race] live companion daemons on server=${daemons}; events=${JSON.stringify(events)}`)
  ...
  test('reinstall (force) while connected: clean single re-connect, no flap', async () => {
    ...
    // Exactly what COMPANION_INSTALL does: drop the live conn, then force-install.
