...
`com.superagent.*` is reserved. Packages using it are only accepted from Cherry's own origins or as builtins bundled with the host, and official apps live under `com.superagent.miniapp.*`. Builtins update with the host, through the same update flow.
