...
const translatePrompt = (locale: string, glossary: Glossary, items: unknown[], style: StyleExample[]): string => {
  ...
  return `Translate SuperAgent UI strings from English into ${LANGUAGE_NAMES[locale]}.

SuperAgent is a desktop AI chat client. Each string below comes with its full i18n key path, which tells you which screen and which kind of control it belongs to — translate for that situation, not for the sentence in isolation.
