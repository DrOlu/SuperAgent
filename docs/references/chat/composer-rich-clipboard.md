...
tokens when users copy and paste between SuperAgent message surfaces and the
composer.
...
- Keep regular clipboard behavior useful outside SuperAgent through
...
| `web application/x-cherry-composer-fragment+json` | Private SuperAgent token fragment. |
...
copies lose token identity after an app restart or in another SuperAgent
