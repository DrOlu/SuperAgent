...
describe('MessageWebSearchToolTitle', () => {
  ...
  it('shows the query in the header and renders each result as a link with favicon and domain', async () => {
    render(
      <MessageWebSearchToolTitle
        toolResponse={{
          ...
          status: 'done',
          arguments: { query: 'SuperAgent' },
          response: [{ id: 1, title: 'SuperAgent', url: 'https://www.superagent.ng/blog', content: 'SuperAgent' }]
        }}
      />
    )
    ...
    expect(within(header).getByText('SuperAgent')).toBeInTheDocument()
    expect(within(header).getByText('1 search results')).toBeInTheDocument()

    fireEvent.click(header)

    const link = await screen.findByRole('link')
    expect(link).toHaveAttribute('href', 'https://www.superagent.ng/blog')
    expect(screen.getByTestId('favicon')).toHaveAttribute('data-hostname', 'www.superagent.ng')
