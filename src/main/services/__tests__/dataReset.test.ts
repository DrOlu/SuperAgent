...
const USER_DATA = '/mock/home/appdata/SuperAgent'
const APP_TEMP = '/mock/tmp/SuperAgent'
const MARKER_FILE = `${USER_DATA}/data-reset.pending.json`
const MARKER_ASIDE = `${USER_DATA}/data-reset.pending.invalid`
const DATABASE_FILE = `${USER_DATA}/Data/superagent.sqlite`
...
const DEFAULT_LISTING = [
  ...
  'superagent.sqlite-personal-backup',
...
function stubApplication(userData: string = USER_DATA, opts: { throwOnUserData?: boolean } = {}) {
  ...
  applicationMock.getPath.mockImplementation((key: string) => {
    ...
    if (key === 'app.database.file') return `${userData}/Data/superagent.sqlite`
...
describe('runDataReset', () => {
  ...
  it("boots normally when no marker file exists in this userData (a sibling instance's marker lives in its own userData and is invisible here)", async () => {
    stubAll(null)
    fsCtl.files.set('/mock/other/SuperAgentDev/data-reset.pending.json', JSON.stringify(pendingMarker()))
    await runReset()
  ...
  it('records the canonical physical path with the arming write', async () => {
    stubAll(pendingMarker())
    realpathNativeMock.mockImplementation(() => '/mock/physical/SuperAgent')
    seedMarker(pendingMarker({ canonicalPath: '/mock/physical/SuperAgent' }))
    await runReset()

    expect(fsCtl.commits[0]).toMatchObject({
      canonicalPath: '/mock/physical/SuperAgent',
      attempts: 1
    })
  })
