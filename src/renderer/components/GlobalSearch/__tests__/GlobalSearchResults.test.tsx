...
beforeEach(() => {
  ...
  data.sessions = {
    'session-1': {
      agentId: 'agent-1',
      workspace: { name: 'SuperAgent', path: 'D:\\code\\superagent', type: 'user' }
    }
  }
...
describe.each([true, false])('global search entry context (recent=%s)', (recent) => {
  ...
  it('shows the agent and named workspace, with the full path available on hover', () => {
    ...
    expect(screen.getByText('CherryClaw | SuperAgent')).toHaveAttribute(
      'title',
      'CherryClaw | D:\\code\\superagent'
    )
  ...
  it('hides system workspace paths and handles deleted owners without inventing a default', () => {
    ...
    expect(screen.queryByText(/SuperAgent/)).not.toBeInTheDocument()
