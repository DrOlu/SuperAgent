...
Unlike other migrators, it writes to a **file-based store** (`~/.superagent/boot-config.json`) rather than a SQLite table. See [Boot Config Overview](../../../../../../docs/references/data/boot-config-overview.md) for why this system exists.
...
|---|---|---|---|
| `~/.superagent/config/config.json` → `appDataPath` | `app.user_data_path` | `Record<string, string>` | *(null — see below)* |
...
- AppImage: `path.dirname(APPIMAGE) + '/superagent.appimage'`
- Windows portable: `PORTABLE_EXECUTABLE_DIR + '/superagent-portable.exe'`
