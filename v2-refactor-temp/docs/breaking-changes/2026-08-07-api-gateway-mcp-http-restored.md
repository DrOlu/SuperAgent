...
The `/v1/mcps*` endpoints removed in v2.0.0 are back on the API gateway, so external clients can again use SuperAgent as a local MCP hub:
...
Users who ran external automation against SuperAgent's MCP servers (browser tooling, scripts, other agent hosts) got a 404 on v2.0.0 and had to stay on 1.9.x. Those integrations work again, with a small edit if they parsed the `{ success, data }` wrapper.
