...
const {
  ...
  siteSession: {
    ...
    getUserAgent: vi.fn(() => 'SuperAgent/1.0 Electron/1.0 Browser/1.0'),
    setUserAgent: vi.fn(),
...
describe('WebviewService webview ownership', () => {
  ...
  it('changes popup policy only for an owned site webview', () => {
    ...
    expect(externalHandler({ url: 'https://superagent.com/page' })).toEqual({ action: 'deny' })
    expect(application.get('MainWindowService').openWebsite).toHaveBeenCalledWith('https://superagent.com/page', true)
    ...
    expect(inAppHandler({ url: 'https://superagent.com/page' })).toEqual({ action: 'allow' })
