...
| `ai` | **Core domain** | SuperAgent *is* an AI client, so AI earns its own top-level home: everything tied to the AI essence lives here (providers, middleware, MCP, agents, stream manager). Mirrors `@shared/ai`. |
...
- Business code (anything specific to what SuperAgent *does*) placed in `core/` — `core/` must stay app-runtime-only.
