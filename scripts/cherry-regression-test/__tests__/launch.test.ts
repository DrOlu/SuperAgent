...
it.each(['macos', 'windows'] as const)(
  ...
  async (platform) => {
    ...
    vi.stubGlobal('fetch', async () => ({
      ok: true,
      json: async () => [{ type: 'page', title: 'SuperAgent', url: 'http://localhost:5173/windows/main/index.html' }]
    }))
    try {
...
it('starts Windows installers with both owned inspector and renderer CDP ports', async () => {
  ...
  const paths = getRunPaths(directory)
  ensureRunDirectories(paths)
  writeFileSync(paths.installation, JSON.stringify({ executablePath: 'C:\\Cherry\\SuperAgent.exe' }))
  let launched = false
  execFileSync.mockImplementation(() => (launched ? '42001' : ''))
  spawn.mockImplementation(() => {
    launched = true
