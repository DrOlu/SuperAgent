...
async function quitWithDataLocationError(cause: unknown): Promise<V2MigrationGateResult> {
  ...
  dialog.showErrorBox(
    ...
    `Could not save the application data directory:\n\n  ${(cause as Error).message}\n\n` +
      `Check that there is free disk space and that ~/.superagent is writable, then try again. The application will now exit.`
  )
  application.quit()
  return 'handled'
}
