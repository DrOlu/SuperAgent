...
export async function sendCherryInCallbackToOwnedApp(record: AppRecord, callback: string): Promise<void> {
  ...
  if (findListeningPid(record.platform, Number(url.port)) !== record.electronPid) {
    throw new Error('Owned SuperAgent instance does not own the OAuth callback listener')
  }
  try {
    ...
    throw new Error('Failed to deliver the OAuth callback to the owned SuperAgent instance')
  }
}
