 ...
 * remove the SuperAgent and Electron from the useragent
 */
export function initSessionUserAgent() {
  ...
  const newUA = originUA.replace(/SuperAgent\/\S+\s/, '').replace(/Electron\/\S+\s/, '')
...
export class WebviewService extends BaseService {
   ...
   * Removes SuperAgent and Electron from the useragent.
   */
