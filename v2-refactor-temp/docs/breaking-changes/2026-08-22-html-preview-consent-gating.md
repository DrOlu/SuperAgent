...
The full-screen HTML preview popup (opened from a code-block card) no longer runs scripts in a same-origin iframe. It now opens in the script-less static tier by default; when the content is active (scripts, embeds, external resources), an explicit **"View webpage"** action appears ( in the Chinese UI) — the same consent wording the inline preview's card already uses — and only that action mounts the hardened isolated webview.
...
Click "View webpage" () when you want an active artifact to execute; nothing else changes.
