...
describe('setOpenLinkExternal', () => {
  ...
  describe('in-app mode (isExternal=false)', () => {
    ...
    it('allows http and https popups', () => {
      expect(handler({ url: 'https://superagent.com/page' })).toEqual({ action: 'allow' })
      expect(handler({ url: 'http://superagent.com/page' })).toEqual({ action: 'allow' })
    })
  ...
  describe('external mode (isExternal=true)', () => {
    ...
    it('still routes safe web URLs to the system browser and denies everything else', () => {
      expect(handler({ url: 'https://superagent.com/page' })).toEqual({ action: 'deny' })
      expect(shell.openExternal).toHaveBeenCalledWith('https://superagent.com/page')
...
describe('the keyboard relay preload', () => {
  ...
  beforeEach(async () => {
    ...
    Object.assign(session, {
      fromPartition: vi.fn(() => ({
        getUserAgent: () => 'Mozilla/5.0 SuperAgent/1.0 Electron/30.0 Safari/537',
        setUserAgent: vi.fn(),
