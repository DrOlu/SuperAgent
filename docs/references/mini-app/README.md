---
description: Entry point for building a SuperAgent mini app — what a package is, the host API, and where each rule lives
sources:
...
A **mini app** is a static web app shipped as a `.miniapp` package (a zip with a `manifest.json` at its root). SuperAgent installs it locally, serves it from `cherry-miniapp://<appId>/` inside a sandboxed `<webview>`, and exposes host capabilities through one global: `window.cherry`.
