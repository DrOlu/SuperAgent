...
When an Agent or assistant pauses for tool approval, SuperAgent now surfaces it the same way it surfaces completions: an in-app card when another conversation is active, or a system notification when the app is in the background. Clicking either takes you straight to the waiting conversation.
