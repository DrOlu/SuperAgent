...
test(...caseDefinition('T-01'), async ({ app, mainWindow: page }) => {
  ...
  await input.fill('SuperAgent Neptune 27182 TRANSLATION_MARKER')
  ...
  await expect(page.getByText('SuperAgent Neptune 27182 TRANSLATION_MARKER', { exact: true }).last()).toBeVisible()
})
