...
describe('navigate protocol handler', () => {
  ...
  it('blocks paths outside the route allowlist', () => {
    handleNavigateProtocolUrl(new URL('superagent://navigate/agents-legacy'))
  ...
  it('opens settings routes through the main-window settings helper', () => {
    handleNavigateProtocolUrl(new URL('superagent://navigate/settings/provider?id=openai'))
  ...
  it('opens non-settings routes with the query string preserved', () => {
    handleNavigateProtocolUrl(new URL('superagent://navigate/agents?x=1&y=2'))
  ...
  it('blocks internal MCP install parameters', () => {
    handleNavigateProtocolUrl(
      new URL('superagent://navigate/settings/mcp/servers?protocolInstall=forged&protocolInstallRequestId=1')
    )
