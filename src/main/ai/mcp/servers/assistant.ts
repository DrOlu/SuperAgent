...
const NAVIGATE_TOOL: Tool = {
  name: 'navigate',
  description:
    'Create a clickable entry for a route returned by product_info. Use this in the same turn whenever answering where to find, open, configure, or use a SuperAgent page or feature; written UI steps are not a substitute.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
...
const DIAGNOSE_TOOL: Tool = {
  name: 'diagnose',
  description:
    'Read SuperAgent runtime state for troubleshooting. Use this to inspect app info, provider config, connectivity, logs, and MCP server status.',
  inputSchema: {
    type: 'object',
    properties: {
      action: {
...
const PRODUCT_INFO_TOOL: Tool = {
  name: 'product_info',
  description:
    'Read current SuperAgent product facts from the installed package manifest. Request only the relevant section to keep context small.',
  inputSchema: {
    type: 'object',
    properties: {
      source: {
...
// Whitelist of settings SuperAgent Assistant can write directly. Each entry binds
...
The tool returns the new agent details, and SuperAgent presents a Go to chat action. Do not call navigate after a successful creation.`,
  inputSchema: {
    type: 'object',
    properties: {
      name: {
      ...
      model: {
        type: 'string',
        description:
          'Optional model id in the form "providerId::modelId" (e.g. "cherryin::agent/glm-5.1", "anthropic::claude-sonnet"). When omitted, the new agent uses SuperAgent Assistant\'s current model.'
      }
    },
...
const APPLY_SETTING_TOOL: Tool = {
  name: 'apply_setting',
  description: `Apply a low-risk SuperAgent setting change directly. Only the whitelist below is supported; destructive operations are never exposed here.
...
const PREPARE_DIAGNOSTIC_REPORT_TOOL: Tool = {
  ...
  description:
    'Prepare an editable diagnostic report description for SuperAgent to present as a user-clickable review action. This tool only prepares draft data; it DOES NOT open UI, acknowledge user consent, collect diagnostics, write files, or submit a report.',
  inputSchema: {
    type: 'object',
    properties: {
      description: {
  ...
  private async createAgent(args: Record<string, string | undefined>) {
    ...
    try {
      ...
      if (isDataApiError(error) && error.code === DataApiErrorCode.NOT_FOUND) {
        throw new McpError(ErrorCode.InvalidParams, `Model is not configured in SuperAgent: ${parsedModel.data}`)
      }
      throw error
    }
