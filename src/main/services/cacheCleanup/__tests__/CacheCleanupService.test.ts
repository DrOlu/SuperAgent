...
describe('CacheCleanupService', () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(app.getPath).mockImplementation((name) => (name === 'exe' ? rootPath('SuperAgent') : '/mock/path'))
    ...
    vi.mocked(application.getPath).mockImplementation((key: string, filename?: string) => {
      const paths: Record<string, string> = {
        ...
        'v1.database.file': path.join(userDataPath, 'superagent.sqlite'),
  ...
  it('does not remove a legacy directory that contains the active userData directory', async () => {
    ...
    const activeDatabase = path.join(userDataPath, 'Data', 'superagent.sqlite')
  ...
  it('counts and removes the root legacy database and Claude config without touching v2 data', async () => {
    const legacyDatabase = rootPath('superagent.sqlite')
    ...
    const v2Database = rootPath('Data', 'superagent.sqlite')
  ...
  it('removes only the current installation mapping from the shared legacy config', async () => {
    const executablePath = rootPath('SuperAgent')
    ...
    await writeTestFile(
      ...
      JSON.stringify({
        appDataPath: [
          ...
          { executablePath: '/other/SuperAgent', dataPath: '/other/data' }
        ],
        retainedField: true
      })
    )
    ...
    expect(updated).toEqual({
      appDataPath: [{ executablePath: '/other/SuperAgent', dataPath: '/other/data' }],
      retainedField: true
    })
  })
  ...
  it('recomputes the shared legacy config update after acquiring its cross-process lock', async () => {
    ...
    const otherEntry = { executablePath: '/other/SuperAgent', dataPath: '/other/data' }
    const concurrentEntry = { executablePath: '/concurrent/SuperAgent', dataPath: '/concurrent/data' }
    bootConfigGet.mockReturnValue({ [executablePath]: root })
    await writeTestFile(
      homeConfigPath,
