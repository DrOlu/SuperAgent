...
This is the main entry point for SuperAgent's WindowManager documentation. WindowManager is a lifecycle-managed service that creates, tracks, and reuses Electron `BrowserWindow` instances with three lifecycle modes (default / singleton / pooled), IPC broadcast, domain-service event hooks, and elastic pool reuse.
...
**SuperAgent windows do not share a single "pin" concept** — the three pinnable windows each mean something different by it:
