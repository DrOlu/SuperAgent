...
Any external client or agent that talked to SuperAgent's MCP servers over HTTP — listing MCP servers, fetching their tools, or connecting to the Claw agent's MCP transport — will get a 404 against the v2 gateway. In-app MCP usage (MCP servers consumed inside SuperAgent's own AI runtime) is unaffected; only the HTTP-exposed surface for external agents was removed.
