...
const lowDiskResult: DoctorCheckResult = {
  ...
  detail: { variant: 'low' },
  evidence: [{ key: 'path', value: '/Users/local/SuperAgent', dataClass: 'local_only' }],
  actions: []
}
...
const translations: Record<string, string> = {
  ...
  'settings.doctor.messages.relaunch_required': 'Restart SuperAgent to apply the repair.',
  ...
  return {
    ...
    report: {
      ...
      basics: {
        ...
        isPackaged: true,
        isPortable: false,
        userDataPath: '/Users/local/SuperAgent'
      },
      results,
...
describe('ErrorDetailContent diagnostics', () => {
  ...
  it('keeps error details open until an in-progress Doctor repair reports its result', async () => {
    ...
    expect(await screen.findByText('Restart SuperAgent to apply the repair.')).toBeInTheDocument()
  ...
  it('shows only user-fixable rows in Diagnostic result without local diagnostic dumps', async () => {
    ...
    expect(within(result).queryByText('/Users/local/SuperAgent')).not.toBeInTheDocument()
