...
7.  **Promote Usage**: Promote SuperAgent through blogs, social media, and other channels to attract more users and developers.
...
Automated tests are triggered on pull requests (PRs) opened by members of the SuperAgent organization, except for draft PRs. PRs opened by new contributors will initially be marked with the `needs-ok-to-test` label and will not be automatically tested. Once a SuperAgent organization member adds `/ok-to-test` to the PR, the test pipeline will be created.
...
v2 is the next major milestone for SuperAgent, and we invite every developer to actively participate! Whether it's new feature development, architecture optimization, or code refactoring, your contributions on `main` are welcome. Let's build a better SuperAgent together!
...
Thank you for your support and contributions! We look forward to working with you to make SuperAgent a better product.
