...
const shellEnvMock = vi.hoisted(() => ({
  getShellEnv: vi.fn().mockResolvedValue({ Path: 'C:\\Users\\me\\.superagent\\bin;C:\\Windows' })
}))
...
describe('McpRuntimeService stdio environment', () => {
  beforeEach(() => {
    ...
    shellEnvMock.getShellEnv.mockResolvedValue({ Path: 'C:\\Users\\me\\.superagent\\bin;C:\\Windows' })
  })
  ...
  it('canonicalizes a mixed-case Windows Path key to PATH before crossing the MCP SDK boundary', async () => {
    ...
    expect(transportEnv?.PATH).toBe('C:\\Users\\me\\.superagent\\bin;C:\\Windows')
