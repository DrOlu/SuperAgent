...
Long-context models no longer compact at Claude Code's generic 200,000-token default when SuperAgent has more accurate model metadata. Editing that metadata also rebuilds the Work Agent connection before the next turn so the new threshold takes effect.
