...
export function runDataReset(): void {
  try {
    ...
    try {
      ...
      showDataResetError(
        'Data Reset Failed',
        'SuperAgent could not record the data reset state ' +
          `in ${markerPath()}.\n\n` +
          'Starting now could erase data you create later, so the app will quit instead.\n\n' +
          'Please check disk space and file permissions, then start SuperAgent again.'
      )
      return
    }
    ...
    if (failures.length > 0) {
      if (operation === 'v1_remigration') {
        ...
        showDataResetError(
          ...
          'SuperAgent could not safely remove the current v2 data required to rerun migration. ' +
            'The app will quit and keep the pending request so the cleanup can retry on the next launch.\n\n' +
            'Please check disk space, file permissions, and antivirus locks, then start SuperAgent again.'
        )
        return
      }
    ...
    try {
      ...
      showDataResetError(
        ...
        'SuperAgent erased its data but could not record the reset as finished ' +
          `in ${markerPath()}.\n\n` +
    ...
    showDataResetError(
      'Data Reset Failed',
      'SuperAgent could not safely complete a pending data reset. ' +
...
function showPathMismatchWarning(): void {
  dialog.showErrorBox(
    ...
    'SuperAgent did not run Data Reset because the data location changed after confirmation.\n\n' +
...
function showIncompleteResetWarning(): void {
  dialog.showErrorBox(
    'Data Reset Incomplete',
    'SuperAgent could not remove some of its data during the data reset.\n\n' +
