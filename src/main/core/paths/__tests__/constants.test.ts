...
const DEFAULT_LOGS = '/default/Logs/SuperAgent'
const DEFAULT_USER_DATA = '/default/userData/SuperAgent'
...
describe('LOGS_DIR dev diversion', () => {
  ...
  it('dev Windows: nests logs under the suffixed userData (win32 semantics)', async () => {
    stubPlatform('win32')
    stubWin32Path()
    const { setAppLogsPath } = stubElectron({ userData: 'C:\\Users\\dev\\AppData\\Roaming\\SuperAgent' })
    const { LOGS_DIR } = await loadConstants()
    vi.doUnmock('node:path')
    const expected = 'C:\\Users\\dev\\AppData\\Roaming\\SuperAgentDev\\logs'
  ...
  it('dev: a traversal suffix aborts startup instead of collapsing LOGS_DIR onto the packaged directory', async () => {
    stubPlatform('darwin')
    vi.stubEnv('CS_DEV_USER_DATA_SUFFIX', '/../SuperAgent')
    stubElectron()
...
describe('CS_DEV_USER_DATA_SUFFIX validation', () => {
  it.each([
    ['/../SuperAgent', 'POSIX traversal'],
    ['\\..\\SuperAgent', 'Windows traversal'],
