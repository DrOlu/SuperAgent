...
vi.mock('@shared/data/presets/miniApps', async (importOriginal) => ({
  ...
  BUILTIN_MINI_APPS: [
    { appId: 'com.superagent.miniapp.notes', name: { en: 'Notes', 'zh-CN': '笔记' }, icon: 'icon.webp' },
...
describe('useBuiltinMiniApps', () => {
  it('offers only the builtins that are not installed yet', () => {
    const { result } = renderHook(() =>
      useBuiltinMiniApps([{ presetMiniAppId: 'com.superagent.miniapp.draw' }], 'zh-CN')
    )

    expect(result.current.map((e) => e.appId)).toEqual(['com.superagent.miniapp.notes'])
  ...
  it('resolves the icon under the shipped resources directory', () => {
    ...
    expect(result.current[0].iconUrl).toBe(
      'file:///opt/cherry/resources/builtin-mini-apps/com.superagent.miniapp.notes/icon.webp'
    )
  })
