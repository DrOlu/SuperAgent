...
// every conversation to (PI_CODING_AGENT_DIR points at <cwd>/.cate/cate-agent):
//
//   <cwd>/.cate/cate-agent/sessions/--<cwd-with-/-as-dashes>--/<timestamp>_<uuid>.jsonl
//
...
const SESSIONS_SEGMENT_POSIX = '/.cate/cate-agent/sessions/'
const SESSIONS_SEGMENT_NATIVE = `\\.cate\\cate-agent\\sessions\\`
...
async function summarizeFile(
  runtime: Runtime,
  hostFilePath: string,
): Promise<CodingSessionListEntry | null> {
  let raw: string
  try {
...
export async function listSessions(cwd: string): Promise<CodingSessionListEntry[]> {
  ...
  const entries: CodingSessionListEntry[] = []
