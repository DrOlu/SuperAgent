...
const sessionPath = '/work/.cate/cate-agent/sessions/--work--/session.jsonl'
...
describe('session listing and deletion safety', () => {
  it('summarizes valid files, ignores unreadable/invalid sessions, and sorts newest first', async () => {
    const dir = '/work/.cate/cate-agent/sessions/--work--'
    const files = {
      [`${dir}/older.jsonl`]: [
  ...
  it('rejects reads and deletes outside the pi sessions directory before resolving a runtime', async () => {
    ...
    await expect(deleteSession('/work/.cate/cate-agent/sessions/readme.txt')).rejects.toThrow('not a pi session file')
    expect(h.resolve).not.toHaveBeenCalled()
  })

  it('routes a remote delete to its owning runtime using the decoded host path', async () => {
    const remotePath = '/srv/work/.cate/cate-agent/sessions/--srv-work--/remote.jsonl'
