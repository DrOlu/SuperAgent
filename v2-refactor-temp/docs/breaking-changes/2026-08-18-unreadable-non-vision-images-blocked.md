...
When an image is attached to a model without Vision enabled, SuperAgent still tries OCR first. If OCR is unavailable, fails, or extracts no readable text, the turn now stops with an actionable error instead of sending the native image to the provider.
