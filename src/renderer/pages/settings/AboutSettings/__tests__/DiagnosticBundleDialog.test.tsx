...
const savedResult: Extract<OutputFor<'diagnostics.bundle.export'>, { status: 'saved' }> = {
  ...
  filePath: AbsoluteFilePathSchema.parse('/tmp/superagent-diagnostics.zip'),
  hasWarnings: false,
  includedFileCount: 2,
  omittedFileCount: 0,
  status: 'saved'
...
describe('DiagnosticBundleDialog', () => {
  ...
  it('reveals the saved file without embedding its local path in the support email', async () => {
    ...
    await waitFor(() =>
      expect(mocks.request).toHaveBeenCalledWith('file.show_in_folder', {
        kind: 'path',
        path: '/tmp/superagent-diagnostics.zip'
      })
    )
    ...
    await waitFor(() => {
      ...
      expect(mailto).toMatch(/^mailto:support@superagent\.ng\?/)
      ...
      expect(decodeURIComponent(mailto)).toContain('superagent-diagnostics.zip')
  ...
  it('falls back to copying the support email when no mail client can be opened', async () => {
    ...
    await waitFor(() => expect(clipboardWrite).toHaveBeenCalledWith('support@superagent.ng'))
