...
const endpoint: NetworkEndpoint = { id: 'update', url: 'https://releases.superagent.ng/release-history.json' }
...
describe('NetworkService.diagnoseEndpoint', () => {
  it('runs all four layers directly and reports reachable', async () => {
    ...
    expect(result).toMatchObject({
      host: 'releases.superagent.ng',
      verdict: 'reachable',
      proxy: { effective: 'DIRECT' }
    })
    expect(probes.resolveHost).toHaveBeenCalledWith('releases.superagent.ng', signal)
    expect(probes.tlsHandshake).toHaveBeenCalledWith('releases.superagent.ng', 443, signal)
  })
