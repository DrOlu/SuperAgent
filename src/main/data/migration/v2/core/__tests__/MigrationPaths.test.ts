 ...
 *   executablePath: D:\SuperAgent\SuperAgent.exe   (v1 custom install dir)
 *   dataPath:       E:\Dropbox\Cherry Data\SuperAgent   (custom data dir, still on disk)
 *
...
const CONFIG_FILE = '/mock/home/.superagent/config/config.json'
...
describe('selectLegacyUserData', () => {
  ...
  it('keeps a fresh Windows portable build isolated from setup data', () => {
    const result = selectLegacyUserData({
      ...
      entries: [{ executablePath: 'C:\\Program Files\\SuperAgent\\SuperAgent.exe', dataPath: '/setup/data' }],
      currentExe: 'D:\\Portable\\superagent-portable.exe',
  ...
  it('keeps different Windows portable locations isolated without an exact mapping', () => {
    const result = selectLegacyUserData({
      ...
      entries: [{ executablePath: 'E:\\OldPortable\\superagent-portable.exe', dataPath: 'E:\\OldPortable\\data' }],
      currentExe: 'D:\\NewPortable\\superagent-portable.exe',
  ...
  it('keeps a fresh Windows setup build isolated from portable data', () => {
    const result = selectLegacyUserData({
      currentUserData: 'C:\\Users\\Alice\\AppData\\Roaming\\SuperAgent',
      entries: [{ executablePath: 'D:\\Portable\\superagent-portable.exe', dataPath: 'D:\\Portable\\data' }],
      currentExe: 'C:\\Program Files\\SuperAgent\\SuperAgent.exe',
...
const REAL_USER_CONFIG = JSON.stringify({
  appDataPath: [
    { executablePath: 'D:\\SuperAgent\\SuperAgent.exe', dataPath: 'E:\\Dropbox\\Cherry Data\\SuperAgent' }
  ]
})
...
describe('resolveMigrationPaths — legacy custom userData recovery', () => {
  it('places v2-managed data under Data while retaining explicit v1 source paths', () => {
    ...
    expect(result.paths.databaseFile).toBe(path.join(DEFAULT_USER_DATA, 'Data', 'superagent.sqlite'))
  ...
  it('redirects to the matching entry when the current exe matches exactly (regression guard)', () => {
    h.normalizedExe.mockReturnValue('D:\\SuperAgent\\SuperAgent.exe')
    applyFs({
      dirs: ['E:\\Dropbox\\Cherry Data\\SuperAgent'],
      contents: {
        [CONFIG_FILE]: REAL_USER_CONFIG,
        [marker('E:\\Dropbox\\Cherry Data\\SuperAgent', 'version.log')]: GOOD_VERSION_LOG
      }
    })

    const result = resolveMigrationPaths()

    expect(result.paths.userData).toBe('E:\\Dropbox\\Cherry Data\\SuperAgent')
    expect(result.userDataChanged).toBe(true)
    expect(result.legacyDataConfirmed).toBe(true)
    expect(h.setPath).toHaveBeenCalledWith('userData', 'E:\\Dropbox\\Cherry Data\\SuperAgent')
  })

  it('recovers the sole recorded dataPath when v2 was reinstalled to a new location (exe no longer matches)', () => {
    h.normalizedExe.mockReturnValue('C:\\Users\\me\\AppData\\Local\\Programs\\superagent\\SuperAgent.exe')
    applyFs({
      dirs: ['E:\\Dropbox\\Cherry Data\\SuperAgent'],
      contents: {
        [CONFIG_FILE]: REAL_USER_CONFIG,
    ...
    expect(result.dataLocation).toBe('E:\\Dropbox\\Cherry Data\\SuperAgent')
    expect(h.setPath).toHaveBeenCalledWith('userData', 'E:\\Dropbox\\Cherry Data\\SuperAgent')
  })
  ...
  it('recovers the only on-disk dataPath among multiple entries when none match the current exe', () => {
    ...
    applyFs({
      dirs: ['/Volumes/Data/SuperAgent'],
      contents: {
        [CONFIG_FILE]: JSON.stringify({
          appDataPath: [
            { executablePath: '/old/portable/exe', dataPath: '/removed/usb/SuperAgent' },
            { executablePath: '/old/install/exe', dataPath: '/Volumes/Data/SuperAgent' }
          ]
        }),
        '/Volumes/Data/SuperAgent/version.log': GOOD_VERSION_LOG
      }
    })

    const result = resolveMigrationPaths()

    expect(result.paths.userData).toBe('/Volumes/Data/SuperAgent')
    expect(result.userDataChanged).toBe(true)
    expect(h.setPath).toHaveBeenCalledWith('userData', '/Volumes/Data/SuperAgent')
  })
  ...
  it('A0: current default already holds a non-empty sqlite + stale eligible entry → no redirect', () => {
    ...
    applyFs({
      ...
      sqlite: { [path.join(DEFAULT_USER_DATA, 'Data', 'superagent.sqlite')]: 4096 },
      contents: {
        [CONFIG_FILE]: JSON.stringify({
  ...
  it('A0 non-empty guard: a 0-byte sqlite does NOT count as V2-ized → still fuzzy-recovers custom', () => {
    ...
    applyFs({
      ...
      sqlite: { [path.join(DEFAULT_USER_DATA, 'Data', 'superagent.sqlite')]: 0 }, // 0 bytes → invalid
      contents: {
        [CONFIG_FILE]: JSON.stringify({
