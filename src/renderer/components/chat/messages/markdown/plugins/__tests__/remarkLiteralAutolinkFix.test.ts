...
describe('remarkLiteralAutolinkFix', () => {
  it('re-pairs emphasis that GitHub/cmark-gfm would leave inside the href (deliberate deviation)', () => {
    expect(
      inlineChildren('PR 已创建：**https://github.com/DrOlu/SuperAgent/pull/19113**（`x` → `y`）。').map(shape)
    ).toEqual([
      { type: 'text', value: 'PR 已创建：' },
      {
        ...
        children: [
          {
            type: 'link',
            url: 'https://github.com/DrOlu/SuperAgent/pull/19113',
            children: [{ type: 'text', value: 'https://github.com/DrOlu/SuperAgent/pull/19113' }]
          }
        ]
      },
      { type: 'text', value: '（' },
