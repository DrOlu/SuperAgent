...
Both dev and packaged runs write to the **app logs directory** (`~/Library/Logs/SuperAgent/` on macOS; `application.getPath('app.logs')`): signals stream into `app.<date>.log`, and the CPU profile lands beside it as `boot-whenReady.cpuprofile`.
