...
export const BUILTIN_AGENT_TOOL_GUARD_RULES: readonly ToolGuardRule[] = [
  ...
  },
  {
    ...
    effect: 'ask',
    reason: 'Submitting SuperAgent feedback externally requires live per-call user approval.',
    headless: {
      predicate: 'either',
      reason:
  ...
  },
  {
    ...
    headless: {
      predicate: 'either',
      reason:
        'Headless channel or scheduled turns cannot present a diagnostic report for user review. Ask the user to open the feedback entry in SuperAgent instead.'
    }
  }
]
