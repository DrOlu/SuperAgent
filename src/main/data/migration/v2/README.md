...
> `~/.superagent/config/config.json`. If migration code calls
...
| `ctx.paths.databaseFile` | `path.join(app.getPath('userData'), 'Data', 'superagent.sqlite')` |
...
| `ctx.paths.legacyConfigFile` | `path.join(os.homedir(), '.superagent', 'config', 'config.json')` |
...
publicly or outside SuperAgent support.
...
by migration mappings. Do not restore whole-state parsing or include `persist:superagent` in the generic
