...
const CHERRYIN_CALLBACK = 'superagent://oauth/callback'
...
async function resolveAuthorizationDestination(
  ...
  for (let redirectCount = 0; redirectCount < 5; redirectCount += 1) {
    if (nextUrl.protocol === 'superagent:') return callbackUrl(nextUrl, expectedState)
...
async function requireCallback(
  ...
  if (destination.protocol !== 'superagent:') throw new Error(errorMessage)
...
async function finishConsent(session: SameOriginSession, nextUrl: URL, expectedState: string): Promise<string> {
  ...
  if (consentUrl.protocol === 'superagent:') return consentUrl.toString()
