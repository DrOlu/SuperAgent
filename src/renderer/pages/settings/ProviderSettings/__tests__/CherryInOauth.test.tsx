...
describe('CherryInOauth', () => {
  ...
  it('renders the logged-in card with balance and footer attribution', async () => {
    ...
    expect(screen.getByText(/superagent\.ng/)).toBeInTheDocument()
  })
  ...
  it('renders the logged-out card when there is no OAuth token', () => {
    ...
    const loginButton = screen.getByRole('button', { name: /Obtain API Key/i })
  ...
  it('opens the Paystack key purchase page for a preset-derived CherryIN provider without starting OAuth', async () => {
    ...
    expect(ipcApiRequestMock).not.toHaveBeenCalledWith(
      'oauth.cancel_sign_in',
      expect.objectContaining({ providerId: 'cherryin' })
    )
  ...
  it('keeps the login action available without reporting an OAuth error after the purchase page opens', async () => {
