...
The pre-existing temp directory (`<temp>/SuperAgent/context-build-vfs`) is removed once at startup. The model-facing `fs_read` tool is now restricted to exactly the persisted outputs referenced by the current conversation (previously: the whole temp directory).
