...
// SuperAgent instance owns that directory, so relocation must not touch it.
...
export function assertRelocationPaths(
  ...
  if (targetExists) {
    ...
    if (ACTIVE_PROFILE_MARKERS.some((marker) => entries.includes(marker))) {
      invalid(
        'target_in_use',
        `target appears to be an active userData directory; close other SuperAgent instances, or remove stale SingletonLock and SingletonSocket markers if none are running: ${toValue}`
      )
    }
  }
