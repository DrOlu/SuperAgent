...
SuperAgent UI component library for React applications.

## ✨ Features

- 🎨 **Design System**: SuperAgent primitive palettes, product semantics, and Shadcn-compatible theme mappings
...
Use the full SuperAgent design system so Tailwind theme tokens resolve to SuperAgent values.
...
- ✅ Colors resolve to SuperAgent design values
...
<Button className="bg-primary text-red-500 p-4 rounded-lg">
  ...
  {/* text-red-500 -> SuperAgent red-500 */}
...
@theme inline {
  --color-primary: var(--cs-brand-500); /* Adopt a SuperAgent foundation value */
...
- ✅ Gives access to SuperAgent foundation values (`var(--cs-brand-500)`, `var(--cs-red-500)`)
- ✅ Lets you choose what to adopt and what to keep
- ✅ Works when you already own the semantic contract and only need selected SuperAgent foundations
- ⚠️ Does not expose the complete Shadcn or SuperAgent product contract
...
SuperAgent product variables extend the same unprefixed public namespace. Use the
...
2. Approved SuperAgent product semantics are also unprefixed, such as `--success` and `--background-subtle`
...
Use the Shadcn CLI to scaffold or update component source and dependency metadata only. SuperAgent's authored
...
- Review any CSS proposed by `shadcn add` and place it according to ownership: official semantics in `shadcn.css`,
  SuperAgent product semantics in `product.css` and `theme-contract.ts`, and component-local styles with the
  component.
...
When SuperAgent consumes the package source, the app's UI-contract generator treats those markers as structural
