...
export async function buildClaudeCodeSessionSettings(
  ...
  // SuperAgent Assistant keeps its existing local-only support MCP behavior. Cherry Support also
...
async function buildToolPermissions(
  ...
  // SuperAgent Assistant feedback skills submit through Bash, so the MCP-only approval list cannot
    ...
    if (interactionState.currentTurn === 'headless' || interactionState.userResponse === 'unavailable') {
      return {
        hookSpecificOutput: {
          ...
          permissionDecision: 'deny',
          permissionDecisionReason:
            'Headless channel or scheduled turns cannot submit SuperAgent feedback. Keep only a sanitized local feedback draft for an interactive user to review and submit.'
        }
      }
    }
    return {
      hookSpecificOutput: {
        hookEventName: 'PreToolUse',
        permissionDecision: 'ask',
        permissionDecisionReason: 'Submitting SuperAgent feedback externally requires live per-call user approval.'
      }
    }
  }
