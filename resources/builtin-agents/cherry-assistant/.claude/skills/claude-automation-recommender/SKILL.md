...
### Phase 0: Confirm Before ScanningSuperAgent addition
...
1. **Announce the scan plan** in one short paragraph: what dirs/files you will
   ...
   > `src/` `tests/`  +  `.claude/`  + CLAUDE.md hook /
   > subagent / skill / MCP  **~30K tokens**

2. **Ask explicit confirmation** with a clear yes/no question — in SuperAgent
   the chat UI surfaces this as a confirmation button:

   > **** / **Proceed with scan?**

3. **Wait for explicit "yes" / "" / "go ahead"** before proceeding to Phase 1.
