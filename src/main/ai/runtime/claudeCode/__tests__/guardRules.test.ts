...
vi.mock('@main/ai/toolApproval/userDataSqliteGuard', () => ({
  USER_DATA_SQLITE_GUARD_REASON: 'Access to SQLite files inside SuperAgent user data is blocked.',
    ...
    it.each(['default', 'acceptEdits', 'bypassPermissions', 'plan', 'auto'] as const)(
      ...
      async (permissionMode) => {
        ...
        await expect(
          ...
          effect: 'deny',
          ruleId: 'user-data-sqlite-write',
          reason: 'Access to SQLite files inside SuperAgent user data is blocked.'
        })
