...
describe('BuiltinAgentProvisioner', () => {
  ...
  it('loads Cherry Support identity from its own package and plugins from SuperAgent Assistant', () => {
  ...
  it('rejects invalid bundled creation defaults', () => {
    writeFile(
      ...
      JSON.stringify({ name: 'SuperAgent Assistant', configuration: { permission_mode: 'invalid' } })
    )
