/**
 * Builds ClaudeCodeSettings from SuperAgent's agent session configuration.
 *
 * Maps SuperAgent's internal data model (agent sessions, providers, MCP servers,
...
const MINIMAL_CHERRY_ASSISTANT_INSTRUCTIONS =
  'Within SuperAgent, serve as SuperAgent Assistant, its built-in general-purpose Agent and onboarding guide. Help the user complete any request using the available tools.'
...
/**
 * Build session-level ClaudeCodeSettings from SuperAgent's agent session.
 */
export async function buildClaudeCodeSessionSettings(
  ...
  // Assistant diagnostics there. Local SuperAgent Assistant sessions keep the full MCP.
...
async function buildToolPermissions(
    ...
    return {
      hookSpecificOutput: {
        ...
        permissionDecisionReason:
          'Headless channel or scheduled turns cannot mutate agent configuration. Ask the user to make this change in SuperAgent.'
      }
    }
  }
  ...
  // SuperAgent Assistant may edit automatically, but it must never turn that convenience into
  ...
  const assistantDestructiveOperationHook: HookCallback = async (input): Promise<HookJSONOutput> => {
    ...
    if (!reason) return {}
    logger.info('Blocked destructive SuperAgent Assistant operation', { sessionId: session.id, toolName, reason })
    return {
      hookSpecificOutput: {
        hookEventName: 'PreToolUse',
        permissionDecision: 'deny',
        permissionDecisionReason:
          `SuperAgent Assistant blocked ${reason}. It must never permanently delete data or bypass this safeguard. ` +
    ...
    if (interactionState.currentTurn === 'headless' || interactionState.userResponse === 'unavailable') {
      return {
        hookSpecificOutput: {
          ...
          permissionDecision: 'deny',
          permissionDecisionReason:
            'Headless channel or scheduled turns cannot submit SuperAgent feedback. Keep the local feedback draft for an interactive user to review and submit.'
        }
      }
    }
    return {
      hookSpecificOutput: {
        hookEventName: 'PreToolUse',
        permissionDecision: 'ask',
        permissionDecisionReason: 'Submitting SuperAgent feedback to Feishu requires live per-call user approval.'
      }
    }
  }
...
export async function buildSystemPrompt(
  ...
  if (builtinRole && !instructions) {
    ...
    if (definition?.instructions) {
      ...
      logger.error('Builtin SuperAgent Assistant definition missing; using minimal fallback instructions')
...
export function buildMcpServers(
  ...
  // 5. Assistant — navigate + diagnose tools (local SuperAgent Assistant sessions only)
  if (assistantMcpEnabled) {
    ...
    logger.debug('SuperAgent Assistant: injected assistant MCP server', {
