...
describe('legacyV1BrowserData', () => {
  ...
  it('uses persisted v1 state or an incomplete cleanup as the visibility marker', () => {
    ...
    localStorage.setItem('persist:superagent', '')
    expect(hasLegacyV1Marker()).toBe(true)

    localStorage.removeItem('persist:superagent')
  ...
  it('estimates selected localStorage keys and every IndexedDB page', async () => {
    localStorage.setItem('persist:superagent', 'legacy')
    ...
    const expectedBytes =
      encoder.encode('persist:superagent').byteLength +
  ...
  it('deletes only the v1 keys and the SuperAgent database', async () => {
    ...
    await vi.waitFor(() => expect(deleteRequest.deleteDatabase).toHaveBeenCalledWith('SuperAgent'))
  ...
  it('keeps the retry marker after a partial run and clears it after a successful retry', async () => {
    ...
    expect(localStorage.getItem('persist:superagent')).toBeNull()
    expect(hasLegacyV1Marker()).toBe(true)
