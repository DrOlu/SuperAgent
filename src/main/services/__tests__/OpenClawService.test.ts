...
describe('OpenClawService gateway status state machine', () => {
    ...
    it.each(['tools.web.fetch.ssrfPolicy', 'gateway.bind'])(
      ...
      async (issuePath) => {
        ...
        await expect(
          ...
          message: expect.stringContaining("outside SuperAgent's managed provider section")
        })
      }
    )
  })
  ...
  describe('syncProviderConfig existing-config handling', () => {
    ...
    it('removes stale Cherry providers while preserving supported hand edits on the selected provider', async () => {
      ...
      const provider = {
        ...legacyProvider,
        headers: { 'User-Agent': 'SuperAgent', 'X-Synced': 'synced' },
        models: [
          {
            id: 'gpt-4o',
            name: 'GPT-4o',
    ...
    it.each([
      ...
      async ({ initialToken }) => {
        ...
        expect(result.success).toBe(false)
        expect('message' in result && result.message).toContain("outside SuperAgent's managed provider section")
