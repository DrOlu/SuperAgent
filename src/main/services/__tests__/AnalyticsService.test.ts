...
describe('AnalyticsService consent revocation', () => {
  ...
  it('discards the pending queue without flushing on revoke', async () => {
    ...
    await expect(injectedFetch!('https://analytics.superagent.ng/api/events')).rejects.toSatisfy(isNonRetriable)
  })
  ...
  it('aborts in-flight requests on revoke with a non-retriable error', async () => {
    ...
    const inFlight = injectedFetch!('https://analytics.superagent.ng/api/events')
  ...
  it('preserves flush behavior on normal stop with consent still valid', async () => {
    ...
    await expect(injectedFetch!('https://analytics.superagent.ng/api/events')).resolves.toBe(response)
