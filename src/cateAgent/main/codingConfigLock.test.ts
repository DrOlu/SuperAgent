...
describe('shared agent config access', () => {
  it('returns null for missing, corrupt, and non-object files', async () => {
    await expect(readCodingConfigFile(file)).resolves.toBeNull()
  ...
  it('preserves existing keys and writes private file permissions where supported', async () => {
    ...
    await updateCodingConfigFile(file, (current) => ({ ...current, owned: 'value' }))

    await expect(readCodingConfigFile(file)).resolves.toEqual({ external: true, owned: 'value' })
  ...
  it('serializes concurrent read-modify-write updates without losing data', async () => {
    await Promise.all(
      Array.from({ length: 12 }, () =>
        updateCodingConfigFile(file, (current) => ({
          ...current,
    ...
    await expect(readCodingConfigFile(file)).resolves.toMatchObject({ count: 12 })
  })
})
