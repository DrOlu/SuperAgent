...
export function apply(ctx: Context): void {
  ...
  // Track the embedding limitation in DrOlu/SuperAgent#19801; execution permissions remain unchanged.
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    try {
      ...
      return {
        kind: 'deny' as const,
        reason: 'The SuperAgent safety guard could not verify this tool call.'
      }
    }
