...
const AUTHORIZATION_URL = `https://open.cherryin.ai/oauth2/auth?state=${STATE}&client_id=superagent`
...
function createOauthFetch(
  ...
  return vi.fn(async (input: string | URL, init: RequestInit = {}): Promise<Response> => {
    ...
    if (url.pathname === '/oauth2/auth') {
      ...
      if (url.searchParams.has('consent_verifier')) {
        return new Response(null, {
          headers: {
            location: `superagent://oauth/callback?code=code-14142&state=${options.callbackState ?? STATE}`
          },
          status: 302
        })
      }
    ...
    if (url.pathname === '/api/oauth/login' && method === 'GET') return jsonResponse({ client_name: 'SuperAgent' })
...
describe('CherryIN OAuth automation', () => {
  it('completes the application flow while preserving the server session cookie', async () => {
    ...
    expect(callback).toBe(`superagent://oauth/callback?code=code-14142&state=${STATE}`)
