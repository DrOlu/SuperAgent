...
describe('product_info', () => {
  ...
  it('returns a compact current-package manifest index through its registered path', async () => {
    const manifest = {
      schemaVersion: 1,
      package: { name: 'SuperAgent', version: '2.0.0-dev' },
      routes: { primary: [], all: [] },
...
describe('create_agent', () => {
  ...
  it("defaults to SuperAgent Assistant's current model when model is omitted", async () => {
  ...
  it('rejects a well-formed model id that is not configured', async () => {
    ...
    ).rejects.toThrow('Model is not configured in SuperAgent: anthropic::missing')
    expect(mocks.agentCreate).not.toHaveBeenCalled()
  })
})
