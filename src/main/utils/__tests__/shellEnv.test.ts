...
vi.mock('@application', () => ({
  application: {
    getPath: (key: string) => {
      if (key === 'cherry.bin') return 'C:\\Users\\test\\.superagent\\bin'
      if (key === 'feature.binary.data') {
        return 'C:\\Users\\test\\AppData\\Roaming\\SuperAgent\\Toolchain\\mise'
      }
...
describe('shellEnv – Windows registry PATH', () => {
  ...
  // -- SuperAgent tool directories appended ------------------------------
  ...
  it('should preserve the unmodified user environment for system tools', async () => {
    ...
    expect(env.Path).not.toContain('.superagent')
  })

  it('should append SuperAgent tool directories to PATH', async () => {
    mockRegistryQuery((args) => {
      const keyPath = args[1]
    ...
    expect(env.Path).toContain('.superagent')
