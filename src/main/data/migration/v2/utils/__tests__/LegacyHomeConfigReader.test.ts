...
const CONFIG_PATH = '/mock/home/.superagent/config/config.json'
  ...
  describe('record return cases', () => {
    ...
    it('converts an array of valid entries into a record', async () => {
      ...
      mockFs.readFileSync.mockReturnValue(
        JSON.stringify({
          appDataPath: [
            { executablePath: '/Applications/SuperAgent.app/exe', dataPath: '/Volumes/Ext1/Data' },
            { executablePath: '/Applications/SuperAgent Dev.app/exe', dataPath: '/Volumes/Ext2/DevData' }
          ]
        })
      )
      ...
      expect(reader.getUserDataPath()).toEqual({
        '/Applications/SuperAgent.app/exe': '/Volumes/Ext1/Data',
        '/Applications/SuperAgent Dev.app/exe': '/Volumes/Ext2/DevData'
      })
    })
