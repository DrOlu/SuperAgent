...
export function buildPathRegistry() {
  ...
  const appTemp = path.join(sysTemp, 'SuperAgent')
  ...
  return Object.freeze({
    // -- A. cherry.* — ~/.superagent infrastructure --
    ...
    'app.database.file': path.join(appUserDataData, 'superagent.sqlite'),
    ...
    // Protocol deep-link (Linux .desktop entry for superagent:// scheme)
    ...
    'v1.database.file': path.join(appUserData, 'superagent.sqlite'),
