...
describe('required DSH Bun runtime', () => {
  it.each([
    [
      'en-US',
      true,
      'DSH could not start because its bundled runtime is unavailable. Reinstall SuperAgent and try again.'
    ],
    [
      'en-US',
      false,
  ...
  it('requires the version marker supplied by the build', async () => {
    ...
    await expect(resolveDshBunRuntime()).rejects.toThrow('Reinstall SuperAgent')
