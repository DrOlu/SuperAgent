...
describe('ChannelMessageHandler', () => {
  ...
  it('confines an image with a hostile media type to channel-images as .png', async () => {
    ...
    try {
      ...
      const written = await readdir(path.join(workDir, '.superagent', 'channel-images'))
      expect(written).toHaveLength(1)
      expect(written[0]).toMatch(/\.png$/)
      expect(await readdir(workDir)).toEqual(['.superagent'])
      expect(await readdir(path.join(workDir, '.superagent'))).toEqual(['channel-images'])
    } finally {
