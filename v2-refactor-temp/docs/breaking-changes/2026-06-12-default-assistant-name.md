...
Fresh v2 databases seed a persisted default assistant backed by the managed CherryAI `cherryai::qwen` model. The seeded assistant name is chosen once from Electron's preferred system languages: Chinese languages (`zh-*`) use `Cherry `, and all other languages use `Cherry Assistant`.
...
Fresh profiles whose preferred system language is Chinese start with `Cherry `; other fresh profiles start with `Cherry Assistant`. The seeded assistant remains ordinary user data and can be renamed or deleted, and it is not automatically renamed if the app language changes later.
