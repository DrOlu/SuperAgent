# SuperAgent Shadcn Variable System
...
This document defines the current variable system for SuperAgent. It is intentionally focused on the Shadcn
...
| unprefixed product semantics | Consumer-backed SuperAgent extensions | Stable public API when Shadcn has no matching role |
...
separate Shadcn and SuperAgent ownership inventories over the values already shipped by SuperAgent:
...
3. unprefixed SuperAgent product semantics in the same public namespace as the Shadcn contract;
...
`product.css` is the authored SuperAgent product layer. Every entry is stable public API. Historical renderer
...
### 3.4 SuperAgent product semantic layer
...
All product variables are stable, consumer-backed SuperAgent semantics not covered by Shadcn.
...
### 4.3 SuperAgent product color extensions
