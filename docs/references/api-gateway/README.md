...
The **API Gateway** exposes SuperAgent's AI capabilities over a local HTTP
...
```json
{"v":2,"t":"superagent-pair","name":"Desktop","port":34444,"ips":["192.168.1.8"],"invitationId":"…","invitationSecret":"…","desktopIdentity":"12D3Koo…","protocolVersions":[1]}
```

`v` is the QR format version; `t` identifies a SuperAgent pairing payload.
