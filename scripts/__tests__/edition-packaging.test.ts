...
describe('edition packaging', () => {
  ...
  it('keeps the existing global product and update identity', () => {
    ...
    expect({
      ...
      appId: 'com.hyperspace.SuperAgent',
  ...
  it('shares the global Electron application name used for userData', async () => {
    ...
    expect(packageMetadata.productName ?? packageMetadata.name).toBe('SuperAgent')
    expect(chinaPackageMetadata.productName ?? chinaPackageMetadata.name).toBe('SuperAgent')
  })
