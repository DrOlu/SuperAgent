...
function buildContextHint(errorInfo: Record<string, unknown>, context?: DiagnosisContext): string {
  ...
  if (status === 401 || msg.includes('api_key') || msg.includes('unauthorized')) {
    const provider = errorInfo.provider || context?.providerName || 'the provider'
    return `## Context\nThe user is calling ${provider} API and got an authentication error. SuperAgent lets users configure API keys per provider in provider settings.\n`
  }
  ...
  if (
    ...
    return `## Context\nNetwork or proxy error. SuperAgent supports HTTP/SOCKS proxy configuration in system settings. The user may be behind a firewall or using a custom API endpoint.\n`
  }
  ...
  // Generic
  return `## Context\nSuperAgent is an AI chat app connecting to LLM providers (OpenAI, Anthropic, Google, Ollama, etc.) with API keys. Error occurred during ${source || 'chat'}.\n`
}
...
export async function diagnoseError(
  ...
  const prompt = `You are an error diagnosis assistant for SuperAgent, an AI chat desktop app.
...
export async function classifyErrorByAI(error: SerializedError, language: string): Promise<string> {
  const prompt = `You are an error diagnosis assistant for SuperAgent. Summarize this error in one sentence (max 30 words) in ${language}. Return ONLY the summary text, no JSON, no markdown, no quotes.`
