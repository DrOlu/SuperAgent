...
function cleanOldOvmsInstallation() {
  ...
  const csDir = path.join(os.homedir(), '.superagent')
...
async function installOvmsBase() {
  ...
  // del %USERPROFILE%\.superagent\ovms_log.log
  // ovms.exe --config_path models/config.json --rest_port 8000 --log_level DEBUG --log_path %USERPROFILE%\.superagent\ovms_log.log
  ...
  try {
    ...
    fs.appendFileSync(runBatPath, 'del %USERPROFILE%\\.superagent\\ovms_log.log\r\n')
    fs.appendFileSync(
      runBatPath,
      'ovms.exe --config_path models/config.json --rest_port 8000 --log_level DEBUG --log_path %USERPROFILE%\\.superagent\\ovms_log.log\r\n'
    )
