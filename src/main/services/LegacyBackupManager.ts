...
class BackupManager {
   ...
   * backups include only Data/superagent.sqlite and cache.json.
   */
   ...
   * Slim backups keep only Data/superagent.sqlite and cache.json.
  ...
  private createBackupFileName(): string {
    return `superagent.${dayjs().format('YYYYMMDDHHmmssSSS')}.${getHostname() || 'unknown'}.${getDeviceType() || 'unknown'}.zip`
  }
  ...
  private async cleanupOldBackups(
    ...
    try {
      ...
      const files = (await listFiles(signal)).filter(
        (file) => file.fileName.startsWith('superagent.') && file.fileName.endsWith(suffix)
      )
  ...
  private async restoreUnlocked(backupPath: string): Promise<void> {
    ...
    try {
      ...
      if (!(await fs.pathExists(path.join(extractionDir, 'metadata.json')))) {
        throw new Error(
          `Unsupported v1 backup. SuperAgent v2 can only restore backup version ${DIRECT_BACKUP_VERSION}.`
        )
      }
  ...
  private async restoreDirect(extractionDir: string): Promise<void> {
    ...
    if (existingJournal.kind === 'corrupt') {
      throw new Error('A corrupt restore journal already exists. Restart SuperAgent before trying again.')
    }
    if (
      existingJournal.kind === 'ok' &&
      (existingJournal.journal.state === 'staged' || existingJournal.journal.state === 'promoting')
    ) {
      throw new Error('Another restore is already pending. Restart SuperAgent before trying again.')
    }
    ...
    try {
      ...
      if (metadata.resources.database) {
        await this.assertArchiveFile(path.join(extractionDir, 'superagent.sqlite'), 'SQLite database')
        await fs.copy(path.join(extractionDir, 'superagent.sqlite'), workDatabase)
      } else {
      ...
      if (!this.isChainBundledPrefix(chain)) {
        throw new Error(
          `${BACKUP_NEWER_VERSION_ERROR_CODE}: This backup was created by a newer version of SuperAgent (database is ahead of this version) and cannot be restored here. Please update SuperAgent and try again. Backup appVersion: ${metadata.appVersion ?? 'unknown'}, current: ${app.getVersion()}.`
        )
      }
      ...
      try {
        ...
        const journal: Extract<RestoreJournal, { state: 'staged' }> = {
          ...
          db: {
            ...
            aside: path.relative(userDataPath, path.join(restoreDir, 'aside', 'superagent.sqlite')),
            fingerprint,
            chain
          },
          fileResources
  ...
  private async readDirectBackupMetadata(extractionDir: string): Promise<DirectBackupMetadata> {
    ...
    if (!raw || typeof raw !== 'object' || raw.appName !== 'SuperAgent') {
      throw new Error('This backup file is not from SuperAgent and cannot be restored')
    }
    if (typeof raw.version === 'number' && raw.version > DIRECT_BACKUP_VERSION) {
      throw new Error(
        `${BACKUP_NEWER_VERSION_ERROR_CODE}: This backup was created by a newer version of SuperAgent (backup version ${String(raw.version)}) and cannot be restored on this version (supports ${DIRECT_BACKUP_VERSION}). Please update SuperAgent and try again.`
      )
    }
    if (raw.version !== DIRECT_BACKUP_VERSION) {
      throw new Error(
        `Unsupported backup version ${String(raw.version)}. SuperAgent v2 can only restore backup version ${DIRECT_BACKUP_VERSION}.`
      )
    }
  ...
  async restoreFromWebdav(_: Electron.IpcMainInvokeEvent, webdavConfig: WebDavConfig) {
    return this.runBackupWorkflow(async () => {
      const filename = webdavConfig.fileName || 'superagent.backup.zip'
      const webdavClient = this.getWebDavInstance(webdavConfig)
  ...
  async restoreFromS3(_: Electron.IpcMainInvokeEvent, s3Config: S3Config) {
    return this.runBackupWorkflow(async () => {
      const filename = s3Config.fileName || 'superagent.backup.zip'
  ...
  async createLanTransferBackup(
    ...
    const fileName = `superagent.${timestamp}.zip`
