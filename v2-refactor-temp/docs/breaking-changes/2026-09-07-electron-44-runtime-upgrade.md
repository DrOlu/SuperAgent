...
The Downloads default comes from Electron itself, not from a SuperAgent setting, so there is no toggle to offer. If users complain, the fix is to track the last-used directory per dialog in the app — worth deciding before release rather than after.
