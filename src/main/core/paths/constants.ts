...
export const CHERRY_HOME_DIRNAME = '.superagent'
...
// (e.g. `/../SuperAgent`). Trailing dots are rejected separately — Windows
// strips them, aliasing `SuperAgent.` (from suffix `.`) onto the packaged
// `SuperAgent` directory. Anything a filesystem accepts inside a single
// component — spaces, non-ASCII — stays valid.
const FORBIDDEN_DEV_USER_DATA_SUFFIX = /[\\/:*?"<>|]|\p{Cc}/u

/**
 * Dev-instance directory suffix (`SuperAgent` → `SuperAgentDev`),
...
/**
 * Dev-instance userData directory (`…/SuperAgent` → `…/SuperAgentDev`).
...
// derives from the app *name* (~/Library/Logs/SuperAgent), elsewhere from
