...
1. For each independent SuperAgent product question, invoke `cherry-assistant-guide` and read the current package through `mcp__assistant__product_info`; never recite product facts from memory.
...
5. Collect and submit SuperAgent feedback through `superagent-feedback`, with Feishu as the default destination. Use `issue-reporter` only when the user explicitly requests a GitHub Issue; generic requests to submit a problem or bug must never trigger `gh`.
