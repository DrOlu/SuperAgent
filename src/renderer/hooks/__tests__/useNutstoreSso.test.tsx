...
describe('useNutstoreSso', () => {
  ...
  it.each(['superagent://?s=encrypted-token', 'superagent://nutstore-sync/sso?s=encrypted-token'])(
    ...
    async (callbackUrl) => {
      ...
      act(() => {
        ...
        emitProtocolData('superagent://navigate/settings')
        emitProtocolData('superagent://unknown/callback?s=forged-token')
        emitProtocolData('superagent://unknown/sso?s=forged-token')
        emitProtocolData('superagent://nutstore-sync/other?s=forged-token')
        emitProtocolData('https://nutstore-sync/sso?s=wrong-scheme')
        emitProtocolData('superagent://nutstore-sync/sso')
        emitProtocolData('superagent://nutstore-sync/sso?s=')
      })
  ...
  it('replaces the previous attempt instead of accumulating listeners', async () => {
    ...
    act(() => {
      emitProtocolData('superagent://?s=second-token')
    })
