...
describe('useDoctorController', () => {
  ...
  it('blocks closing while opening the displayed app data directory and releases it afterwards', async () => {
    ...
    act(() => {
      opening = result.current.openPath('/Users/local/SuperAgent')
    })

    expect(mocks.request).toHaveBeenCalledWith('system.shell.open_path', '/Users/local/SuperAgent')
  ...
  it('opens the existing application logs directory from the advanced tools', async () => {
    ...
    mocks.request.mockImplementation(async (route: string) =>
      route === 'app.get_info' ? { logsPath: '/Users/local/SuperAgent/logs' } : undefined
    )
    const { result } = renderHook(() =>
      useDoctorController({ subject: { kind: 'global' }, initialPanel: 'checks', onNavigate: vi.fn() })
    )
    ...
    expect(mocks.request).toHaveBeenCalledWith('system.shell.open_path', '/Users/local/SuperAgent/logs')
  })
  ...
  it('executes every non-fix backend action with its exact public contract', async () => {
    ...
    await act(async () =>
      result.current.executeAction('network-endpoint-update', {
        kind: 'open_external',
        url: 'https://superagent.ng/status'
      })
    )
    ...
    expect(mocks.request).toHaveBeenCalledWith('system.shell.open_external_website', 'https://superagent.ng/status')
