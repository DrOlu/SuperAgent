 ...
 * home config file (~/.superagent/config/config.json) sources, then writes
 * values to bootConfigService (~/.superagent/boot-config.json).
 */
...
export class BootConfigMigrator extends BaseMigrator {
  ...
  private loadMigrationItems(): MigrationItem[] {
    ...
    const configFileMappings: ReadonlyArray<{ originalKey: string; targetKey: BootConfigKey }> = [
      {
        // `appDataPath` field at the top level of ~/.superagent/config/config.json
