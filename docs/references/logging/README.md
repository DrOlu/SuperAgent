...
SuperAgent uses a unified logging service to print and record logs. **Unless there is a special reason, do not use `console.xxx` to print logs**.
...
There are many log levels. The following are the guidelines that should be followed in SuperAgent for when to use each level:
