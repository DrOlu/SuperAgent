...
vi.mock('./codingDir', () => ({ sharedAuthPath: () => '/virtual/auth.json' }))
...
vi.mock('./codingConfigLock', () => ({
  // Return a snapshot immediately. Code that reads outside updateCodingConfigFile
  // can therefore take the same stale snapshot in two concurrent mutations.
  readCodingConfigFile: vi.fn(async () => ({ ...h.data })),
  // Model the real per-file lock: updater callbacks execute serially and see
  // the latest committed contents.
  updateCodingConfigFile: vi.fn((_path: string, update: (current: JsonObject) => JsonObject) => {
